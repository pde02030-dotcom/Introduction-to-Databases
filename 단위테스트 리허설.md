...
FROM
    course c
        LEFT JOIN
    examInfo ei ON c.course_id = ei.course_id
WHERE
    ei.exam_id IS NULL;
```
...
FROM 
    student s 
        JOIN
    score sc ON s.student_id = sc.student_id -- R1(student join score)
        JOIN
    examInfo ei ON sc.exam_id = ei.exam_id -- R2(R1 join examinfo)
        JOIN
    course c ON ei.course_id = c.course_id; -- R3(R2 join course)
```
## CASE WHEN
```sql
SELECT 
...
FROM
    ...
    Student s ON sc.student_id = s.student_id
        JOIN
    ExamInfo ei ON sc.exam_id = ei.exam_id;
```
